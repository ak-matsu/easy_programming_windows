...
start = [False]
stats = {host: {'min': None, 'max': None, 'total': 0, 'count': 0, 'loss': 0} for host in ['127.0.0.1', get_local_ip(), get_default_gateway(), get_default_gateway(), '8.8.8.8', '8.8.4.4']}
def toggle_start():
    start[0] = not start[0]
    if start[0]:
        button.config(text='停止', bg='red')
        for host, text in zip(['127.0.0.1', get_local_ip(), get_default_gateway(), get_default_gateway(), '8.8.8.8', '8.8.4.4'], texts):
